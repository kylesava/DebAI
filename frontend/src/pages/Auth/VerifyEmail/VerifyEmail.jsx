...
import "../Auth.css"
import { emailConfirmationApi, sentEmailToVerifyEmailApi } from "../../../utils/Api"
...
const VerifyEmail = () => {
    ...
    const handleSendEmail=async(e)=>{
        ...
        try {
            const {status} = await sentEmailToVerifyEmailApi(emailRef.current.value)
            if(status===200){
                ...
                <div className="verify_main_content">
                      <form onSubmit={handleSendEmail} className='email_sent_content_box'>
                        ...
                        <p className='email_desc'>
                          Check you email we will  sent an email that contains a link to verify your email address.</p>

                        </div>
                        <input ref={emailRef} placeholder="your email address to verify" type="email" required/>
        
                       <button  className='verify_email_button' type="submit">SEND EMAIL</button>

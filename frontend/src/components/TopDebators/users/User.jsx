...
const User = ({ user }) => {
  ...
  const getFlags = async () => {
    ...
    if ((countryData, status === 200)) {
      setFlag(
        countryData.find((country) => country?.name?.common === user?.country)
          ?.flags?.svg
      );
    }
  };
  ...
  return (
    <div
      ...
      className={`${styles.top_user} ${user?._id === data?._id && styles.own}`}
    >
      <div className={styles.main_info}>
        <img
          referrerPolicy="no-referrer"
          className={styles.top_user_avatar}
          src={user.avatar}
